...
def execucao_modelos(instancias, modelos, prefixo_arq = "t_minlafav_",  tempo_max=120, fl_primeira_sol = False, fl_heuristica_desabilitada=False, inst_mod_pular=[]):
    ...
    arquivo_solucao = "solucoes/"+prefixo_arq+ "_results.csv"
    f = open(arquivo_solucao, "w")
    f.write("problema;  modelo;  ub_cmax; num_jobs;  num_maquina;  fl_inteiro;  best_bound;  funcao_objetivo;  mip_relative_gap; number_of_contraints; number_of_variables; number_of_var_values; nb_iterations; nb_nodes_processed; nb_linear_nonzeros; status; time; has_hit_limit \n")
    f.close()
    df_solucoes_intermed = pd.DataFrame()
    for i in instancias: 
        id_inst = i["id"]
            ...
            for mod in modelos:
                for ub in [-1.0, ub_cmax]:
                    ...
                    prefixo = prefixo_arq\
                            ...
                            +"_"+modelo.name\
                            +"_ub"+str(abs(int(ub)))
                    print("Modelo "+ prefixo + " criado! com UB do cmax = ", ub_cmax)
                    #modelos.append(modelo)
                    print("############", nome_problema+"_ZInt"+str(fl_inteiro)+"_"+modelo.name)
                    # Pulando instancias-modelo necessarios
                    if nome_problema+"_ZInt"+str(fl_inteiro)+"_"+modelo.name not in inst_mod_pular:                
